...
import EmailWorker from "@service/email/email-worker.js";
...
class AuthService {
  ...
  login = transaction.use(
    async (
      ...
      try {
        const existingUser = await User.findOne(email ? { email } : { phoneNumber }, { session });
  ...
  deleteUser = async (email: string) => {
    return transaction.use(async (session: ClientSession, email: string) => {
      try {
        const user = await User.deleteOne({ email }, { session });
