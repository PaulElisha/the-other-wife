...
class CustomerService {
 getCustomerProfile = async (customerId: number, userId: number) => {
  const [customerProfile] = await db
   ...
   .where(and(eq(customers.id, customerId), eq(users.id, userId)))
   .limit(1);
 ...
 deleteCustomerProfile = async (customerId: number, userId: number) => {
  try {
   const [deletedCustomerProfile] = await db
    .delete(customers)
