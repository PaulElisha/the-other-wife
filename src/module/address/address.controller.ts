...
import AddressService, { labelType } from "@module/address/address.service.js";
...
class AddressController {
 createUserAddress = asyncHandler(
  async (
   ...
   try {
    const userAddress = await AddressService.createUserAddress(userId, {
     city,
     state,
     country,
     postalCode,
 ...
 editUserAddress = asyncHandler(
  async (
   ...
   const userId = Number(req.user.id);
   try {
    const userAddress = await AddressService.editUserAddress(
     userId,
     addressId,
     req.body,
    );
 ...
 toggleDefaultAddress = asyncHandler(
  async (
   ...
   const userId = Number(req.user.id);
   try {
    const defaultAddress = await AddressService.toggleDefaultAddress(
     userId,
     addressId,
    );
