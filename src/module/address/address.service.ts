...
export type labelType = (typeof label)[keyof typeof label];
...
class AddressService {
 createUserAddress = async (
  ...
  await db.transaction(async (tx) => {
   ...
   await tx
    ...
    .where(
     and(eq(vendors.address_id, newAddress.id), eq(vendors.user_id, userId)),
    );
  });
 ...
 editUserAddress = async (
  ...
  await db.transaction(async (tx) => {
   ...
   body.city !== undefined && (updateData.city = body.city),
    ...
    body.postalCode !== undefined && (updateData.postalCode = body.postalCode),
 ...
 toggleDefaultAddress = async (userId: number, addressId: number) => {
 ...
 getUserAddresses = async (userId: number) => {
  const user_address = await db
   .select()
   .from(addresses)
   .where(eq(addresses.id, userId))
