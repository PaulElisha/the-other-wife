...
class VendorService extends VendorBase<VendorSchema> {
 getVendorProfile = async (vendorId: number, userId: number) => {
  const vendorProfile = await db
   .select()
   .from(vendors)
   .innerJoin(users, eq(vendors.user_id, userId))
   .innerJoin(addresses, eq(vendors.address_id, addresses.id))
   .where(and(eq(vendors.id, vendorId), eq(users.id, userId)))
   .limit(1);
 ...
 deleteVendorProfile = async (vendorId: number, userId: number) => {
  try {
   const [deletedVendorProfile] = await db
    .delete(vendors)
    .where(and(eq(vendors.id, vendorId), eq(vendors.user_id, userId)))
    .returning();
