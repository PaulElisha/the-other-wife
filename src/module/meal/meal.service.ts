...
import { mealcategories, meals } from "@module/meal/meal.schema.js";
 ...
 createMeal = async (
  ...
  const meal = await db.transaction(async (tx) => {
   const [vendor] = await tx
    .select()
    .from(vendors)
    .where(and(eq(vendors.user_id, userId), eq(vendors.id, vendorId)))
    .limit(1);

   const [meal_category] = await tx
    .select()
    .from(mealcategories)
    .where(eq(mealcategories.category, categoryName))
    .limit(1);
