...
export class AuthController {
  ...
  handleSignup = handleAsyncControl(
    async (
      ...
      try {
        const handleSignup = this.authService.signup([userType as keyof typeof CreateProfile]);

        const { accessToken, refreshToken, ...userWithoutPassword } = await handleSignup({
          firstName,
          lastName,
          email,
          password,
  ...
  verifySignup = handleAsyncControl(
    async (req: Request<{}, {}, {}, { token: string }>, res: Response): Promise<any> => {
      ...
      try {
        const userWithoutPassword = await this.authService.verifySignup(emailToken);
  ...
  handleLogin = handleAsyncControl(
    async (
      ...
      try {
        const { accessToken, refreshToken, ...userWithoutPassword } = await this.authService.login({
          phoneNumber,
          email,
          password,
        });
